...
def _topk(scores, K=40):
    ...
    topk_score, topk_ind = torch.topk(topk_scores.permute(1,0).contiguous().view(-1), K)
    ...
    #return topk_scores.view(-1), topk_inds.view(-1), topk_ys.view(-1), topk_xs.view(-1)


def decode_bbox_from_heatmap(heatmap, boxes, x_shifts, y_shifts, z_shifts,
                                score_thresh=0.3, K=100, nms_thresh=0.01):
    
    scores, inds, class_id, ys, xs = _topk(heatmap, K=K)
    distance = boxes[:,0:2]
    center_z = boxes[:,2]
    sides = boxes[:,3:6]
    rot_cos = boxes[:,6]
    ...
    boxes_list.append(class_id.float())
    ...
    keep = torch.tensor(keep).cuda().long()
    
    boxes_keep = _gather_feat(cur_boxes, keep)
    scores_keep = _gather_feat(cur_scores, keep)
    ...
    final_labels = boxes_keep[mask][:,-1]
    ret_pred_dicts = {
        'pred_boxes': [],
        'pred_scores': [],
        'pred_labels': []
